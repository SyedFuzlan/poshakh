...
document.addEventListener('keydown', e => {
  const loginScreen = document.getElementById('login-screen');
  if (e.key === 'Enter' && loginScreen && loginScreen.style.display !== 'none') login();
});
...
function showApp(email) {
  ...
  const emailDisplay = document.getElementById('owner-email-display');
...
async function loadStats() {
  try {
    ...
    const ids = {
      'stat-today-orders': s.today_orders,
      'stat-today-revenue': s.today_revenue_formatted,
      'stat-pending': s.pending_shipment,
      'stat-pending-proc': s.pending_processing,
      'stat-low-stock': s.low_stock_count,
      'stat-total': s.total_orders
    };
...
window.loadOrders = async function loadOrders() {
  ...
  try {
    ...
    if (list) list.innerHTML = '<div class="empty-state">Failed to load orders.</div>';
  }
}

window.exportOrdersCSV = function exportOrdersCSV() {
...
window.updateOrderStatus = async function updateOrderStatus(id, status, extra = {}) {
  try {
...
window.markShipped = async function markShipped(id, btn) {
...
window.toggleLowStockFilter = function toggleLowStockFilter() {
  showLowStockOnly = !showLowStockOnly;
  const btn = document.getElementById('btn-low-stock');
  if (btn) btn.classList.toggle('active', showLowStockOnly);
  loadProducts();
}
...
window.toggleStock = function toggleStock(cb) {
  const row = cb.closest('.size-row');
  const stockInput = document.getElementById('stock-' + cb.value);
  if (cb.checked) {
    if (stockInput) stockInput.style.display = 'inline-block';
...
window.saveProduct = async function saveProduct() {
  ...
  if (checkedSizes.length === 0) {
    if (sizesError) sizesError.style.display = 'block';
    return;
  }
  if (sizesError) sizesError.style.display = 'none';
...
// ── Image preview ─────────────────────────────────────
window.previewImages = function previewImages(input) {
...
window.handleDrop = function handleDrop(e) {
  e.preventDefault();
...
async function loadSettings() {
  try {
    ...
    const ids = {
      'setting-announcement': s.announcement_text || '',
      'setting-hero-img': s.hero_banner_image || '',
      'setting-sale-mode': s.is_sale_mode || 'false'
    };
    for (const [id, val] of Object.entries(ids)) {
      const el = document.getElementById(id);
      if (el) el.value = val;
...
window.emToggleStock = function emToggleStock(cb) {
  const row = cb.closest('.size-row');
  const stockCtrl = document.getElementById('em-stock-ctrl-' + cb.value);
  if (cb.checked) {
    if (stockCtrl) stockCtrl.style.display = 'flex';
    row.style.borderColor = 'var(--maroon)';
    row.style.background = 'var(--cream-warm)';
  } else {
    if (stockCtrl) stockCtrl.style.display = 'none';
...
window.adjustStock = function adjustStock(size, delta) {
...
window.openEditModal = async function openEditModal(productId) {
...
window.saveEditModal = async function saveEditModal(productId) {
...
let moItems = [];
window.loadMoVariants = async function loadMoVariants(pid) {
...
window.addMoItem = function addMoItem() {
...
function renderMoItems() {
  ...
  const totalSpan = document.getElementById('mo-total');
  if (totalSpan) totalSpan.textContent = total.toLocaleString('en-IN');
}

window.submitManualOrder = async function submitManualOrder() {
...
window.closeManualOrderModal = function closeManualOrderModal() {
  const modal = document.getElementById('manual-order-overlay');
  if (modal) modal.remove();
  moItems = [];
}

// ── Order History ────────────────────────────────────
window.openHistoryModal = async function openHistoryModal(orderId) {
  try {
...
// ── Bulk Update ──────────────────────────────────────
window.openBulkUpdateModal = function openBulkUpdateModal() {
  const overlayHtml = `
...
window.submitBulkUpdate = async function submitBulkUpdate() {
  ...
  try {
    ...
    const modal = document.getElementById('bulk-update-overlay');
    if (modal) modal.remove();
    loadProducts();
    loadStats();
  } catch (e) { alert(e.message); }
...
window.toggleNewStock = function toggleNewStock(cb) {
  const row = cb.closest('.size-row');
  const ctrlId = cb.id.replace('size-', 'new-stock-ctrl-');
  ...
  if (cb.checked) {
    ...
    if (ctrl) ctrl.style.display = 'none';
    const stockId = cb.id.replace('size-', 'stock-');
...
window.adjustNewStock = function adjustNewStock(sz, delta) {
...
window.addCustomSize = function addCustomSize(containerId, inputId, prefix) {
  ...
  const html = `
    <div class="size-row" style="display:flex;align-items:center;justify-content:space-between;gap:8px;background:var(--cream-warm);border:1px solid var(--maroon);border-radius:4px;padding:8px 12px;margin-bottom:8px">
      ...
      <div id="${prefix}new-stock-ctrl-${val}" style="display:flex;align-items:center;gap:4px">
        ...
        <input type="number" id="${prefix}stock-${val}" min="0" value="0" style="width:50px;height:28px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px"/>
        <button type="button" onclick="${prefix ? 'adjustStock' : 'adjustNewStock'}('${val}', 1)" style="width:28px;height:28px;border:1px solid #ddd;background:#fff;border-radius:4px;cursor:pointer;font-weight:bold">+</button>
      </div>
    </div>`;
